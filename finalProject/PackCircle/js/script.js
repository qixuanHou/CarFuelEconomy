...
function drawAll(bar_data, id_parent_level, pack_data) {
	...
	var padding = 5,
		width = window.innerHeight*0.95, //Math.max($("#chart").innerWidth(),350) - padding,
	...
	var colorCircle = d3.scale.ordinal()
			.domain([0,1,2,3])
			.range(['#f2f2f2','#b3b3b3','#4c4c4c','#1c1c1c']);
	...
	var nodes = pack.nodes(pack_data),
		root = pack_data,
		focus = root,
	...
	bar_data.forEach(function(d) { d.value = +d.value; });
	...
	id_parent_level.forEach(function (d, i) { 
		IDbyName[d.name] = d.ID; 
	});

	// Function to get the color of the bars based on the MPG values
	...
	//var legendCols = ['rgb(126, 185, 205)', 'rgb(108, 175, 198)', 'rgb(90, 165, 191)', 'rgb(71, 156, 184)', 'rgb(64, 140, 165)', 'rgb(57, 124, 147)', 
	//				  'rgb(50, 109, 129)', 'rgb(43, 93, 110)', 'rgb(36, 78, 92)', 'rgb(29, 62, 73)', 'rgb(22, 46, 55)', 'rgb(15, 31, 36)'];

	var legendCols = ['#daebf1', '#b6d7e2', '#91c3d4', '#6cafc6', '#479cb8', '#397c93',
					  '#326d81', '#2b5d6e', '#244e5c', '#1d3e49', '#152f37', '#0e1f25', '#071012'];
	var rangeCols = [11, 15, 19, 23, 27, 31, 35, 39, 41, 45, 49, 51, 55, 59];

	createLegend(legendCols);

	// Function to create legend of the bar chart
			...
			if(node.ID in dataById) {
				...
				if(node.ID.lastIndexOf(currentID, 0) === 0  & !hidden) {
					...
					if(drawTitle & showText) {	
						...
						//chosenContext.fillText("Avg MPG", nodeX, nodeY + -0.9 * nodeR);
						
						//Get the text back in pieces that will fit inside the node
						var titleText = getLines(chosenContext, node.name, nodeR*2*0.6, fontSizeTitle, titleFont);
						...
						titleText.forEach(function(txt, iterator) { 
							...
							chosenContext.textAlign = "center";
							chosenContext.textBaseline = "middle"; 
							chosenContext.fillText(txt, nodeX, nodeY + (-0.75 + iterator*0.125) * nodeR);
						})//forEach
					...
					if (Math.round(barHeight) > 0) {
						...
						for (var j = 0; j < bars.length; j++) {
							...
							bar.barPiecePosition = nodeY + barChartHeightOffset*nodeR + j*eachBarHeight - barChartHeight*nodeR*0.9;
							
							//console.log('bar color'+getColor(bar.value));
							//Draw the bar
							chosenContext.beginPath();
							chosenContext.fillStyle = getColor(bar.value);
							chosenContext.fillRect(nodeX + -nodeR*0.25, bar.barPiecePosition, bar.width, barHeight);
							chosenContext.fill();
							...
							if(drawLabelText & showText) {
								...
								chosenContext.textBaseline = "middle"; 
								chosenContext.fillText(bar.name, nodeX + -nodeR*0.3, bar.barPiecePosition+0.5*barHeight);
							}//if
							...
							bar.posX = nodeX + -nodeR*0.25;
							bar.posY = bar.barPiecePosition;
							bar.height = barHeight
							bar.mfr = node.name.split(" - ")[0];
							allBars.push(bar);
		...
		if(node) {
			...
			if (focus === node) {
		        ...
		        while(bar = allBars[i++]) {
		        	...
		        	hiddenContext.rect(bar.posX, bar.posY, bar.width, bar.height);
			        if (hiddenContext.isPointInPath(cx, cy)) {
			        	if(focus.depth == 2) {
			        		isBar = true;
			        		// console.log(bar.mfr)
			            	updateRadar(bar.mfr, bar.name);
			        	}
			            break;
			        }
		    	}
	...
	if (!mobileSize) {
		...
		var mousemoveFunction = function(e){
			...
			var xoff = document.getElementById('canvas').offsetLeft;
			var yoff = document.getElementById('canvas').offsetTop;
			...
			if(node !== nodeOld) {
				...
				if(node) {
					...
					//if(typeof node.ID !== "undefined") {
						...
						$(".popoverWrapper").css({
							'position':'absolute',
							'top': mouseY + yoff-5, //nodeY-nodeR,
							'left': mouseX + xoff //nodeX+padding*5/4
						});
	...
	function zoomToCanvas(focusNode) {
		...
		// Highlight the nodes in scatterplots
		if (focus.name != "Manufacturers") {
			// in manufacturer name level 
			if (focus.name.indexOf(" - ") == -1) {
				...
				d3.selectAll(".dot")
		            ...
		            .duration(500)
		            .filter(function(d) {
		              return (d["Mfr Name"] != focus.name.split(" - ")[0] || d["Carline Class Desc"] != focus.name.split(" - ")[1]);
		            })
		            .attr("visibility", "hidden");
			}
		} else {
	...
	//Create options - all the pack_data
